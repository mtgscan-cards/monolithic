...
def download_and_extract_resources():
    ...
    if not os.path.exists(run_dir):
        ...
        if response.status_code == 200:
            ...
            with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                ...
                if common_prefix.endswith("/") or common_prefix.endswith("\\"):
                    for member in tqdm(members, desc="Extracting resources"):
                        ...
                        target_path = os.path.join(base_dir, new_path)
                        if member.is_dir():
            ...
            os.remove(zip_path)
            print("Resources extracted to", base_dir)
        else:
...
def download_and_extract_resources_once():
    base_dir = os.path.abspath("resources")
    lock_file_path = os.path.join(base_dir, "resource_download.lock")
    os.makedirs(base_dir, exist_ok=True)
...
def load_resources():
    ...
    hf = h5py.File(os.path.join("resources", "run", "candidate_features.h5"), 'r')
    # Load label mapping from the SQLite "cards" table.
    conn = sqlite3.connect(os.path.join("resources", "run", "card_database.db"))
    label_mapping = pd.read_sql_query("SELECT * FROM cards", conn)
    # Query the FAISS mapping from the SQLite mapping table instead of a text file.
    mapping_df = pd.read_sql_query("SELECT * FROM faiss_mapping", conn)
    conn.close()
    # Load the FAISS index.
    faiss_index = faiss.read_index(os.path.join("resources", "run", "faiss_ivf.index"))
    
    # Construct the index-to-card mapping list from the database.
    # We assume the "faiss_mapping" table has columns "faiss_index" and "scryfall_id".
